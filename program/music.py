...
@Client.on_message(command(["تشغيل", f"play@{BOT_USERNAME}","شغل","ش"]) & other_filters)
async def play(c: Client, m: Message):
    await m.delete()
    do = requests.get(f"https://api.telegram.org/bot5430285495:AAFaqvQ5u4CHw62VkegEEoA5ZjNanlqQxCc/getChatMember?chat_id=@vvvvzzv&user_id={m.from_user.id}").text
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        try:
            ...
            if b.status == "kicked":
                await m.reply_text(
                    f"@{ASSISTANT_NAME} **محظور في المجموعة** {m.chat.title}\n\n» **قم بفك حظر المستخدم أولاً إذا كنت تريد استخدام هذا البوت.**"
                )
                return
        ...
        if replied:
            if replied.audio or replied.voice:
                ...
                if len(m.command) < 2:
                    ...
                    suhu = await c.send_message(chat_id, "🔍**تم يࢪوحي جارر البحث**")
                ...
                if search == 0:
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                await suhu.edit("🔄 **جاࢪ الانضمام إلى المكالمه...**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioPiped(
                                        ytlink,
